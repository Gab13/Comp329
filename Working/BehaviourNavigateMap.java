...
public class BehaviourNavigateMap implements Behavior {
	...
	public void action() {
		...
		if(Assignment.isObstacleInNextTile(estimatedLocation) || (estimatedLocation[0] == unexplored[0] && estimatedLocation[1] == unexplored[1])) {
			...
			Assignment.printMessage("New tile: " + unexplored[0] + "," + unexplored[1]);
		}
		...
		while(nextCell[0] == -1 || nextCell[1] == -1) {
			Assignment.printMessage("Forcing update, current next cell: " + nextCell[0] + "," + nextCell[1]);
			unexplored = Assignment.getUnexploredGridCell(estimatedLocation,  true);
			nextCell = Assignment.getNextCell(estimatedLocation);
		}

		// Where is the next cell in comparison with the current one?
		Assignment.printMessage("Comparing [" + estimatedLocation[0] + "," + estimatedLocation[1] + "] with [" + nextCell[0] + "," + nextCell[1] + "]");		Assignment.printMessage("Comparing [" + estimatedLocation[0] + "," + estimatedLocation[1] + "] with [" + nextCell[0] + "," + nextCell[1] + "]");
		Assignment.printMessage("Current pose " + h);
		...
		if(estimatedLocation[0] == nextCell[0]) {
			if(estimatedLocation[1] == (nextCell[1] + 1)) {
				...
				if(h >= 45 && h < 135) {
					...
					rotate = 180;
					distance = Assignment.GRID_WIDTH - 5;
